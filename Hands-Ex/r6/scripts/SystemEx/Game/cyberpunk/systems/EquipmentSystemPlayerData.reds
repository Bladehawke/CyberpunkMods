...
private func OverloadSystemReplacementCW() -> Void {
	...
	let handAreaIndex: Int32 = this.GetEquipAreaIndex(gamedataEquipmentArea.HandsCW);
	if ArraySize(this.m_equipment.equipAreas[handAreaIndex].equipSlots) < 1 {
		return;
	}

	let handNumSlots: Int32 = SystemEx.NumberOfSlots();
	let emptyEquipSlot: SEquipSlot;
	while ArraySize(this.m_equipment.equipAreas[handAreaIndex].equipSlots) < handNumSlots {
		ArrayPush(this.m_equipment.equipAreas[handAreaIndex].equipSlots, emptyEquipSlot);
	}
}
